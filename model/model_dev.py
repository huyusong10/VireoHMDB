...
class VireoDev(nn.Module):
    ...
    def __init__(self, num_classes, bn_mom = 0.9, bn_eps=0.001, expand_ratio=2):
        super().__init__()
        _, self.conv2d, self.conv3d = get_conv()
        self._bn_mom = 1 - bn_mom
        self._bn_eps = bn_eps
        ...
        dilation = 2
        dilate_pow = (dilation) ** (1 / (len(repeats) - 1))
        stage_expand_ratios = []
        ...
        self._stem = self.Fused_Transform3D(3, channels[0], kernel_size=(5, 5, 5), stride=(1, 2, 2), expand_ratio=1)
        ...
        for idx in range(len(kernel_size)):
            ...
            block = transform(channels[idx], channels[idx+1], kernel_size=kernel_size[idx], stride=strides[idx], expand_ratio=stage_expand_ratios[idx])
            self._blocks.append(block)
            for i in range(repeats[idx] - 1):
                block = transform(channels[idx+1], channels[idx+1], kernel_size=kernel_size[idx], stride=1, se=se[idx], expand_ratio=stage_expand_ratios[idx])
                self._blocks.append(block)
