...
class VireoDev(nn.Module):
    ...
    def __init__(self, num_classes, bn_mom = 0.9, bn_eps=0.001, expand_ratio=5):
        super().__init__()
        _, self.conv2d, self.conv3d = get_conv()
        self._bn_mom = 1 - bn_mom
        self._bn_eps = bn_eps
        ...
        for idx in range(len(kernel_size)):
            ...
            block = transform(channels[idx], channels[idx+1], kernel_size=kernel_size[idx], stride=strides[idx], expand_ratio=1)
            self._blocks.append(block)
            rest_repeats = repeats[idx] - 1
            expand_pow = self.expand_ratio ** (1 / rest_repeats)
            for i in range(rest_repeats):
                expand_ratio = expand_pow ** (i + 1)
