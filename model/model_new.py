...
class VireoNew(nn.Module):
    ...
    def __init__(self, num_classes, bn_mom = 0.9, bn_eps=0.001, expand_ratio=4):
        ...
        ratio_reduction = 1 / 2 
        min_ratio = max_ratio * ratio_reduction
        stage_ratio_pow = (1 / ratio_reduction) ** (1 / (len(repeats)))
        channels = [stem_chann]
        stage_ratios, instage_ratios = [min_ratio], [[min_ratio]]

        for i, block_repeats in enumerate(repeats):
            stage_ratio = min_ratio * stage_ratio_pow ** (i + 1)
            stage_ratios.append(stage_ratio)

            instage_ratio_pow = ratio_reduction ** (1 / (max(1, block_repeats - 2)))
            instage_ratio = [stage_ratio * instage_ratio_pow ** x for x in range(block_repeats - 1)]
            instage_ratios.append(instage_ratio)

            channel = round(channels[i] * max(instage_ratios[i]) * 2 / max(instage_ratios[i+1]))
            channels.append(channel)     

        # print(channels)
        # print(stage_ratios)
        # print(instage_ratios)
        ...
        for idx in range(len(kernel_size)):
            ...
            block = transform(channels[idx], channels[idx+1], kernel_size=kernel_size[idx], stride=strides[idx], expand_ratio=instage_ratios[idx][-1])
            self._blocks.append(block)
            rest_repeats = repeats[idx] - 1
            for i in range(rest_repeats):
                block = transform(channels[idx+1], channels[idx+1], kernel_size=kernel_size[idx], stride=1, se=se[idx], expand_ratio=instage_ratios[idx+1][i])
                self._blocks.append(block)
