...
class Transform3D(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = inputs[:, :self.inc] if self.stride == 1 else inputs[:, -self.inc:]
        shortcut = inputs[:, self.inc:] if self.stride == 1 else None

        # depthwise conv
        x = self._depthwise_conv(x)
        x = self._bn1(x)
        depth_x = self._activate(x)

        if shortcut is not None:
            x = torch.cat((depth_x, shortcut), dim=1)
        # se
        if self._se:
            self._attention(x)
        ...
        if self.stride == 1:
            x = torch.cat((depth_x, shortcut, x), dim=1)
        return x
...
class DenseVireoV4(nn.Module):
    ...
    def __init__(self, num_classes, bn_mom = 0.9, bn_eps=0.001):
        ...
        input_chann = 3
        channels = [24, 24, 48, 96, 112, 192]
        self.last_chann = channels[-1]

        self._stem = self.Fused_Transform3D(input_chann, channels[0], kernel_size=(3, 3, 3), stride=(1, 2, 2), expand_ratio=2)
